    ...
    <div class="comment" id="parent_{{ $comment->id }}">
        @else
            <div class="comment reply-comment" id="comment-{{ $comment->id }}">
                ...
                <div class="payload">
                    <div class="info">
                        <a target="_blank" href="{{ route('profile.overview', $comment->user->name) }}"
                           class="username">
                            {{ $comment->user->name }}
                        </a>
                ...
                {{ formatNumber($comment->likes - $comment->dislikes) }}
                                    ...
                                    <ul>
                                        <li>
                                            @if ($comment->user_id == auth()->user()->id)
                                                <button type="button" @if ( ! isset($reply)) data-type="parent"
                                                        @else data-type="child"
                                                        @endif data-url="{{ route('comments.delete', $comment->id) }}"
                                                        data-id="{{ $comment->id }}"
                                                        class="btn btn-text delete">{{ __('delete') }}</button>
                                            @endif
                                        </li>
                                        <li>
                                            @if ($comment->user_id != auth()->user()->id)
                                                <button type="button" @if ( ! isset($reply)) data-type="parent"
                                                        @else data-type="child"
                                                        @endif data-url="{{ route('comments.report', $comment->id) }}"
                                                        data-id="{{ $comment->id }}"
                                                        class="btn btn-text report">{{ __('report') }}</button>
                                            @endif
                                        </li>
                                    </ul>
                                </div> <!-- end of dropdown -->
                    ...
                    <div class="actions">
                        <button type="button" data-post="{{ $comment->post_id }}"
                                data-parent="{{ $comment->parent_id ?: $comment->id }}" data-id="{{ $comment->id }}"
                                data-name="{{ $comment->user->name }}" class="btn btn-text btn-reply-comm">
                            {{  __('Reply') }}
                        </button>
