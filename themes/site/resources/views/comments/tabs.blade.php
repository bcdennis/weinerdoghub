<ul class="comments-tabs">
    ...
    @if (setting('comments.facebook.on'))
        <li id="tab2" class="tab2 @if ( ! setting('comments.local.on')) active @endif" onclick='FB.XFBML.parse();'>
            <a href="#" id="fbBtn">
                {{ __('Facebook Comments') }} (<span class="fb-comments-count" data-href="{{ route('post', $post->slug) }}" data-version="v2.3">0</span>)
            </a>
        </li>
    @endif
    @if (setting('comments.disqus.on'))
        <li id="tab3" class="tab3">
            <a href="#" id="disBtn">
                {{ __('Disqus Comments') }} (<span class="disqus-comment-count" data-disqus-url="{{ route('post', $post->slug) }}">0</span>)
            </a>
        </li>
    @endif
</ul>
